...
from parameters_py.mgconfig import (
					...
					BOUNDARY_1_SHP,BOUNDARY_2_SHP,EXT_FIG,DPI_FIG,OUTPUT_DIR,DEPTH_MOHO,DEPTH_LAB,Ps_OR_Sp_PHASE
				   )
...
if Ps_OR_Sp_PHASE == 'Ps':
	ev.filter('lowpass',freq=1,corners=4, zerophase=True)

if Ps_OR_Sp_PHASE == 'Sp':
	ev.filter('lowpass',freq=1/6,corners=4, zerophase=True)
...
for i,j in enumerate(ev):
	if j.stats.sac.gcarc > 30:
		...
		sta_dic['sta_baz'].append(round(float(j.stats.sac.baz),3))
		sta_dic['sta_knetwk'].append(j.stats.sac.knetwk)
		sta_dic['sta_kstnm'].append(j.stats.sac.kstnm)
