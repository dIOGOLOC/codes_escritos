...
x, y = m(lons,lats)
sc = m.scatter(x,y,40,RF_number,cmap='binary',marker='s',edgecolors='none')
...
l2, = m.drawgreatcircle(AB_lon_line[0],AB_lat_line[0],AB_lon_line[1],AB_lat_line[1],linewidth=2,color='w')
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
x, y = m(lons,lats)
sc = m.scatter(x,y,40,RF_delta_1_Vp_mean,cmap='magma',marker='s',edgecolors='none',vmin=-0.5,vmax=0.5)
...
cax = divider.append_axes("right", size="5%", pad=0.05)
x, y = m(lons,lats)
sc = m.scatter(x,y,40,RF_DEPTH_true_thickness_MTZ_Pds,cmap='magma',marker='s',edgecolors='none',vmin=250-DEPTH_RANGE,vmax=250+DEPTH_RANGE)
fig.colorbar(sc,cax=cax)
...
for _i, _j in enumerate(RF_data_profile_Ppds):
	...
	if abs(abs(AB_lon_line[1]) - abs(AB_lon_line[0])) > abs(abs(AB_lat_line[1]) - abs(AB_lat_line[0])):
		ax6.text(_i/factor_Ppds*0.95,310,"{0:.1f}".format(AB_lon[_i]),rotation=-45,fontsize=10)
		ax6.set_xlabel('Longitude ($^\circ$)',labelpad=30)
	else:
		ax6.text(_i/factor_Ppds*0.95,310,"{0:.1f}".format(AB_lat[_i]),rotation=-45,fontsize=10)
