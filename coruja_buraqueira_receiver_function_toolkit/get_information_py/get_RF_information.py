...
from parameters_py.config import (
					DIR_SAC,OUTPUT_JSON_FILE_DIR,GAUSSIAN_FILTER,RADIAL_EXT,TRANSVERSAL_EXT,RF_PERCENT,REST_TRACE_CHECK_AMP,
					EV_GCARC_MIN,EV_GCARC_MAX,EV_MAGNITUDE_MB,CUT_BEFORE_P,CUT_AFTER_P,SAMPLING_RATE
				   )
...
for i,j in enumerate(datalistS):
	...
	data_R = data_RF[0].data.tolist()
	data_T = data_RF_T[0].data.tolist()


	#RF P-arrival amplitudes check
	P_arrival_start = round(CUT_BEFORE_P*SAMPLING_RATE)
	P_arrival_end = round((CUT_BEFORE_P*SAMPLING_RATE)+SAMPLING_RATE)

	

	if data_RF[0].stats.sac.user0 == GAUSSIAN_FILTER and data_RF[0].stats.sac.user5 > RF_PERCENT and min(data_RF[0].data) > REST_TRACE_CHECK_AMP and data_RF[0].data[P_arrival_start] > 0 and data_RF[0].data[P_arrival_end] > 0 :

		dic_RF['dataR'].append(data_RF[0].data.tolist())
		dic_RF['dataT'].append(data_RF_T[0].data.tolist())
