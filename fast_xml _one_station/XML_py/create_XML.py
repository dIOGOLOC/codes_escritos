...
print('Importing '+kstnm+' station parameters')
print('\n')

if len(sensor_keys) != 0 and len(accer_keys) == 0 and len(hydrophone_keys) == 0:
	    ...
	    response = nrl.get_response(sensor_keys = sensor_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
...
if len(sensor_keys) == 0 and len(accer_keys) != 0 and len(hydrophone_keys) == 0:
	    ...
	    response = nrl.get_response(sensor_keys = accer_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
...
if len(sensor_keys) != 0 and len(accer_keys) != 0 and len(hydrophone_keys) == 0:
	    ...
	    response = nrl.get_response(sensor_keys = sensor_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
	    response_accer = nrl.get_response(sensor_keys = accer_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
...
if len(sensor_keys) != 0 and len(accer_keys) == 0 and len(hydrophone_keys) != 0:
	    ...
	    response = nrl.get_response(sensor_keys = sensor_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
	    response_hydro = nrl.get_response(sensor_keys = hydrophone_keys.split('*'),datalogger_keys = datalogger_keys.split('*'))
...
inv.write(OUTPUT_XML_FILE_DIR+NETWORK_CODE+'.'+kstnm+".xml", format="stationxml", validate=True)
print(inv)
print('\n')
