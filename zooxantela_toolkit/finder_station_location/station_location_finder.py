...
AZIMUTHAL_GAP_VALUE = 130
SECONDARY_AZIMUTHAL_GAP_VALUE = 160
...
def grid_maker(data_longitude, data_latitude, west, east, south, north):
...
def plot_grid_obs(x_OBS,y_OBS,grx,gry):
    #Figure
...
for i,j in enumerate(tqdm(sta_OBS_lon, desc='OBS location loop')):
    #Creating the grid:
    grx,gry = grid_maker(sta_OBS_lon[i], sta_OBS_lat[i],LLCRNRLON,URCRNRLON,LLCRNRLAT,URCRNRLAT)

    #Plotting the map:
    plot_grid_obs(sta_OBS_lon[i], sta_OBS_lat[i],grx,gry)
    ...
    print(' - OBS network location: lon:'+str(sta_OBS_lon[i])+', lat:'+str(sta_OBS_lat[i]))
    print(' - Number of possible combinations: ',len(comb))
    azimuthal_test_lst = []
    for x,d in enumerate(tqdm(comb, desc='Combinations loop')):
    ...
    for azi in tqdm(azimuthal_test_lst, desc='Approved combinations loop'):
        ...
        plot_grid_obs(sta_OBS_lon[i], sta_OBS_lat[i],azi_gap_gx,azi_gap_gy)
    print('\n')
