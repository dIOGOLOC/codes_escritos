...
Phase_P100s_folder = PP_DIR+'P100s/'
print('Phase_P100s_folder = '+Phase_P100s_folder)
os.makedirs(Phase_P100s_folder,exist_ok=True)
...
Phase_P250s_folder = PP_DIR+'P250s/'
print('Phase_P250s_folder = '+Phase_P250s_folder)
os.makedirs(Phase_P250s_folder,exist_ok=True)
...
input_list_100 = [(i+1,PHASES[0],event_depth[i],event_lat[i],event_long[i],sta_lat[i],sta_long[i],Phase_P100s_folder) for i,j in enumerate(event_depth)]
...
input_list_250 = [(i+1,PHASES[2],event_depth[i],event_lat[i],event_long[i],sta_lat[i],sta_long[i],Phase_P250s_folder) for i,j in enumerate(event_depth)]
...
# ===================================
# Calculating P100s Piercing Points
# ===================================
...
with Pool(processes=MP_PROCESSES,maxtasksperchild=2) as pool_100:
	pool_100.starmap(parallel_piercing_points, input_list_100,chunksize=1)
pool_100.close()
pool_100.join()
...
# =============================
# Saving Piercing Points P100s
# =============================

filename_pds_json_P100s = sorted(glob.glob(Phase_P100s_folder+'*'))

PP_dic_P100s_files = [json.load(open(i)) for i in filename_pds_json_P100s]

PP_dic_P100s = {'depth':[],'time':[],'lat':[],'lon':[]}

for i,j in enumerate(PP_dic_P100s_files):
	PP_dic_P100s['time'].append(j['time'])
	PP_dic_P100s['depth'].append(j['depth'])
	PP_dic_P100s['lat'].append(j['lat']) 
	PP_dic_P100s['lon'].append(j['lon']) 


print('Saving Piercing Points in JSON file')

with open(PP_DIR+'PP_'+PHASES[0]+'_dic.json', 'w') as fp:
	json.dump(PP_dic_P100s, fp)
...
start_time = time.time()
with Pool(processes=MP_PROCESSES,maxtasksperchild=2) as pool_TARGET:
	pool_TARGET.starmap(parallel_piercing_points, input_list_TARGET,chunksize=1)
pool_TARGET.close()
pool_TARGET.join()
print("--- %.2f execution time (min) ---" % ((time.time() - start_time)/60))
...
# ===================================
# Calculating P250s Piercing Points
# ===================================
...
start_time = time.time()
with Pool(processes=MP_PROCESSES,maxtasksperchild=2) as pool_250:
	pool_250.starmap(parallel_piercing_points, input_list_250,chunksize=1)
pool_250.close()
pool_250.join()
print("--- %.2f execution time (min) ---" % ((time.time() - start_time)/60))
...
# =============================
# Saving Piercing Points P250s
# =============================

filename_pds_json_P250s = sorted(glob.glob(Phase_P250s_folder+'*'))

PP_dic_P250s_files = [json.load(open(i)) for i in filename_pds_json_P250s]

PP_dic_P250s = {'depth':[],'time':[],'lat':[],'lon':[]}

for i,j in enumerate(PP_dic_P250s_files):
	PP_dic_P250s['time'].append(j['time'])
	PP_dic_P250s['depth'].append(j['depth'])
	PP_dic_P250s['lat'].append(j['lat']) 
	PP_dic_P250s['lon'].append(j['lon']) 


print('Saving Piercing Points in JSON file')

with open(PP_DIR+'PP_'+PHASES[2]+'_dic.json', 'w') as fp:
	json.dump(PP_dic_P250s, fp)