...
INPUT_DIR = '/home/sysop/dados_posdoc/MTZ_2024/PRF_selected_YES_PP_FILTER_POST/BP.TUTU/'
...
RF_stack_lst = np.array([op.read(i)[0].data for i in RF])
RF_stack = np.sum(RF_stack_lst, axis=0)/len(RF_stack_lst)

# # Organizando as Funções do Receptor em função do backazimuth (°)

gcarc_lst = ([op.read(i,headonly=True)[0].stats.sac.baz for i in RF])
...
ax1.set_xlim(-5,160)
ax1.text(0.93,0.98, 'n:'+str(len(RF)),transform=ax1.transAxes, fontsize='xx-large')
...
plt.show()
plt.savefig(OUTPUT_DIR+'FIGURES_YES_PP_FILTER_POST/RF_'+RF_data.stats.network+'_'+RF_data.stats.station+'_ALL_BAZ.png',dpi=300)
